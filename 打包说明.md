...
- 临时把文本里的 `Shigure` 和 `Fuyutsui` 都替换成输入的新名称，包括插件的 `.lua` 和 `.toc` 文件
- 递归重命名名称中包含 `Shigure` 或 `Fuyutsui` 的文件和目录，例如项目文件、C# 文件、插件目录和插件 `.toc` 文件
- 运行 `dotnet publish`
- 发布结束后恢复所有文本、文件名和目录名
...
正常情况下脚本会在 `dotnet publish` 结束后恢复项目和插件名称。如果恢复失败，先检查根目录是否残留类似 `新名称.csproj`、`新名称.slnx` 或以新名称命名的插件目录，再重新运行脚本或手动改回 `Shigure` / `Fuyutsui` 名称。
