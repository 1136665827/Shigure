...
> [!abstract] AI 快速摘要
> `PixelScanner` 用窗口标题完全匹配目标，截取屏幕上客户区的可见像素。顶行用 `(R,G,B)` 精确编码步骤和值：`R=0` 表示步骤 1..255，`R=1` 表示 256..510，`G` 是分段步骤号，`B` 是值。客户区左边缘的红色锚点定位动作条计数行，其后六行编码最多 30 个单位的治疗吸收。没有颜色容差、没有 `PrintWindow`，因此遮挡、最小化、缩放或着色都可能破坏读取。

## 图谱位置

- 上级：[[docs/30-Shigure/00-Shigure-MOC]]
- 生产/消费权威契约：[[docs/40-跨项目/01-Shingen-像素生产消费契约]]
- 下游状态构建：[[docs/30-Shigure/03-Shigure-配置合并与GameState构建]]
- Fuyutsui 生产者入口：[[docs/20-Fuyutsui/03-Fuyutsui-状态块与编码入口]]；吸收细节见 [[docs/20-Fuyutsui/07-Fuyutsui-队伍与治疗吸收]]
...
| 输入 | 输出字段 | 语义 |
|---|---|---|
| 完全匹配的窗口标题 | `RowData: Dictionary<int,int>?` | 顶行步骤号到 0..255 值 |
...
生产端刷新时序见 [[docs/20-Fuyutsui/02-Fuyutsui-事件与刷新调度]]；吸收值含义见 [[docs/20-Fuyutsui/07-Fuyutsui-队伍与治疗吸收]]。

## 截图和窗口边界

- `FindWindow` 使用**完全相同的标题**，不校验进程 ID、可执行路径或签名。
- 最小化窗口会被拒绝。
...
因此同标题窗口可能成为读取目标；目标被其他窗口覆盖、游戏滤镜改变颜色、Windows 缩放处理异常或插件纹理偏移，都可能造成错误或空扫描。

## 失败模式与排障

| 症状 | 优先检查 |
|---|---|
| 一直“等待像素” | 窗口标题、是否最小化、步骤 1 是否在顶行前 510 像素、Fuyutsui 是否加载 |
...
一个重要语义：配置里声明了字段但本次 `RowData` 没有该步骤时，`StateBuilder` 会把布尔设为 false、字符串设为空、数字设为 0。因此“缺失像素”不总等价于条件系统中的“未知值”；详见 [[docs/30-Shigure/03-Shigure-配置合并与GameState构建]]。

## 修改影响

- 改步骤编码、锚点颜色、510 上限或吸收网格，必须同步 Fuyutsui 生产端和 [[docs/40-跨项目/01-Shingen-像素生产消费契约]]。
...
- `Shigure/Runtime/PixelScanner.cs:7-18`：结果结构、510 上限、255 分段、6×30 吸收常量。
- `Shigure/Runtime/PixelScanner.cs:21-31`：DPI 感知。
- `Shigure/Runtime/PixelScanner.cs:34-96`：窗口检查、客户区坐标、三类扫描结果。
- `Shigure/Runtime/PixelScanner.cs:99-135`：步骤 1 搜索和顶行扫描。
- `Shigure/Runtime/PixelScanner.cs:137-203`：计数条锚点与分段。
- `Shigure/Runtime/PixelScanner.cs:206-251`：六行治疗吸收。
- `Shigure/Runtime/PixelScanner.cs:315-372`：屏幕截图、精确颜色与步骤解码。

## 知识图谱链接

- 上游 Fuyutsui：[[docs/20-Fuyutsui/03-Fuyutsui-状态块与编码入口]]
- 契约中心：[[docs/40-跨项目/00-Shingen-跨项目契约-MOC]]
- 下游：[[docs/30-Shigure/03-Shigure-配置合并与GameState构建]]、[[docs/30-Shigure/04-Shigure-运行循环触发模式与快照]]
- 相关原始说明：[[Shigure/README]]
