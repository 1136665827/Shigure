...
summary: "描述 Fuyutsui 与 Shigure 的职责边界、端到端运行闭环、配置生成链路、关键不变量和跨组件修改影响。"
aliases:
  - "Shingen 系统架构"
...
系统主闭环是：WoW 事件和 API 驱动 Fuyutsui 状态，Fuyutsui 将状态绘制成屏幕像素；Shigure 截屏解码为 `GameState`，用模块规则和 keymap 得出热键，再把按键发回 WoW。除此之外，Shigure 还能定位游戏目录中的 Fuyutsui，编辑 Lua 配置和宏，并生成自己的 `config/*.json` 与 `keymap/*.json`。

因此存在三条必须同步维护的跨项目接口：

1. [[docs/40-跨项目/01-Shingen-像素生产消费契约|屏幕像素生产消费契约]]。
2. [[docs/40-跨项目/02-Shingen-ClassBlocks到config同步契约|ClassBlocks 到 config 同步契约]]。
3. [[docs/40-跨项目/03-Shingen-ClassMacros到keymap与按键契约|ClassMacros 到 keymap 与按键契约]]。
...
| Shigure Lua 同步子系统 | Fuyutsui `class/*.lua`、`core/classmacros.lua` | 更新后的 Lua、`config/*.json`、`keymap/*.json`、运行时重启 |
...
1. Shigure 从目标窗口找到游戏进程，再定位 `Interface/AddOns/Fuyutsui`。
2. 配置页通过 `ClassBlocksStore` 读写 `class/*.lua` 的表字面量；宏页通过 `ClassMacrosStore` 读写 `core/classmacros.lua`。
3. 保存只替换对应 Lua 表，尽量保留文件其他内容和行尾注释。
4. `FuyutsuiConfigConverter` 重新生成 `config`；`FuyutsuiKeymapConverter` 重新生成 `keymap`。
5. 主窗口串行等待同步任务完成，刷新编辑目录并重启运行会话。
...
| 编辑器保存成功，运行结果未变化 | 转换任务或运行时重启未完成，或编辑了非当前游戏目录 |
...
完整验收见 [[docs/40-跨项目/04-Shingen-兼容性变更检查清单|兼容性变更检查清单]]。
...
- `Shigure/App/Program.cs`、`ShigureRuntimeFactory.cs`：组合根和依赖组装。
- `Shigure/App/RuntimeSessionCoordinator.cs`：会话串行化。
- `Shigure/Runtime/PixelScanner.cs`：屏幕采集。
- `Shigure/Runtime/StateBuilder.cs`、`GameState.cs`：状态转换。
- `Shigure/Runtime/ShigureRuntime.cs`：触发和逻辑循环。
- `Shigure/Modules/ModuleStore.cs`、`LogicRegistry.cs`：规则执行和回退。
- `Shigure/Input/KeymapService.cs`、`KeySender.cs`：热键解析和发送。
- `Shigure/Infrastructure/*Fuyutsui*Converter.cs`：两条生成链路。
...
flowchart LR
  subgraph Game["WoW 进程"]
    ...
    ClassBlocks["ClassBlocks"] --> Blocks["运行时 blocks"]
    FState --> Pixels["主色块 / CountBars / 吸收网格"]
    Blocks --> Pixels
    ClassMacros["ClassMacros"] --> Secure["SecureActionButton / 覆盖绑定"]
  end
  ...
  Sender --> Secure
  Secure --> API
  ClassBlocks --> ConfigConverter["ConfigConverter"] --> Config
  ClassMacros --> KeyConverter["KeymapConverter"] --> Keymap
  Editor["Shigure 配置 / 宏编辑器"] --> ClassBlocks
  Editor --> ClassMacros
```
...
- 接口：[[docs/40-跨项目/00-Shingen-跨项目契约-MOC|跨项目契约 MOC]]
