...
summary: "定义 Fuyutsui 职业 ClassBlocks 的声明与索引顺序，及 Shigure 将其转换为 config 并构建 GameState 的一致性要求。"
aliases:
...
related:
  ...
  - "[[docs/20-Fuyutsui/03-Fuyutsui-状态块与编码入口|Fuyutsui 状态块与编码入口]]"
...
像素 RGB 本身由 [[docs/40-跨项目/01-Shingen-像素生产消费契约|像素生产消费契约]] 定义；宏配置由 [[docs/40-跨项目/03-Shingen-ClassMacros到keymap与按键契约|ClassMacros 契约]] 定义。
...
| 找错游戏目录 | UI 编辑和本地生成成功，但实际运行 AddOn 未变化 |
...
- 更新 [[docs/TEXTURE_LAYOUT_zh-CN|纹理排序说明]]及本契约。
...
| Lua 子集解析 | `Shigure/Infrastructure/LuaLiteParser.cs` |
| 可视化配置 round-trip | `Shigure/Infrastructure/ClassBlocksStore.cs` |
| Lua 到 JSON 转换 | `Shigure/Infrastructure/FuyutsuiConfigConverter.cs` |
| config 合并 | `Shigure/Infrastructure/ConfigService.cs` |
| config 到状态 | `Shigure/Runtime/StateBuilder.cs` |
...
- 生产模型：[[docs/TEXTURE_LAYOUT_zh-CN|Fuyutsui 纹理排序说明]]、[[docs/20-Fuyutsui/03-Fuyutsui-状态块与编码入口|状态块与编码入口]]
- 消费模型：[[docs/30-Shigure/03-Shigure-配置合并与GameState构建|配置合并与 GameState]]
- 编辑链路：[[docs/30-Shigure/09-Shigure-Fuyutsui配置宏编辑与同步|Fuyutsui 配置宏编辑与同步]]
- 相邻契约：[[docs/40-跨项目/01-Shingen-像素生产消费契约|像素生产消费契约]]
