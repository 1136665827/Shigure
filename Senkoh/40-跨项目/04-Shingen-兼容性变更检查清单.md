...
related:
  ...
  - "[[docs/OPTIMIZATION_zh-CN|Fuyutsui 历史优化审计]]"
source_files:
...
| 变更 | 契约入口 | 典型影响 |
|---|---|---|
| 主色块数量、RGB、标记或物理布局 | [[docs/40-跨项目/01-Shingen-像素生产消费契约|像素契约]] | `core/block.lua`、PixelScanner、StateBuilder、截图验证 |
| `states/auras/spells/group` 结构或顺序 | [[docs/40-跨项目/02-Shingen-ClassBlocks到config同步契约|ClassBlocks 契约]] | LoadPlayerBlocks、Store、Converter、config、module 字段 |
| 宏顺序、动态占位、热键池 | [[docs/40-跨项目/03-Shingen-ClassMacros到keymap与按键契约|宏与热键契约]] | CreateMacro、KeymapConverter、keymap、实际绑定 |
...
- [ ] 阅读 [[docs/10-系统/00-Shingen-双项目系统全景|双项目系统全景]]和受影响契约。
...
- Fuyutsui README 曾把已删除的 `auracontainer.lua` 列入结构；README 与 [[AGENTS|根协作约定]] 已于 2026-08-09 按当前 TOC 修正，当前加载事实以 `Fuyutsui.toc` 为准。
- Shigure README 曾写 `UnitMappingVersion = 2`；已于 2026-08-09 修正为当前版本 3。迁移事实以 `ModuleStore.cs` 与 `ReservedUnit.cs` 为准。
- [[docs/OPTIMIZATION_zh-CN|Fuyutsui 优化建议]] 是拆分前源码的静态审计，保留历史价值，但其中 `main.lua` 千行行号不能用于当前定位。
- [[docs/AuraContainer_AI_Reference_zh-CN|AuraContainer 参考]] 固定到特定 PTR build；正式服或新构建的 API 结论必须重新验证。
...
| 检查面 | 当前入口 |
|---|---|
| 加载与版本 | `Fuyutsui/Fuyutsui.toc`、`Shigure/Shigure.csproj` |
| 像素生产/消费 | `Fuyutsui/core/block.lua`、`Shigure/Runtime/PixelScanner.cs` |
| ClassBlocks 双实现 | `Fuyutsui/main.lua`、`Shigure/Infrastructure/FuyutsuiConfigConverter.cs` |
| ClassMacros 双实现 | `Fuyutsui/core/macro.lua`、`Shigure/Infrastructure/FuyutsuiKeymapConverter.cs` |
| 状态消费 | `Shigure/Runtime/StateBuilder.cs`、`Modules/ModuleStore.cs` |
| 单位迁移 | `Shigure/Modules/ReservedUnit.cs`、`ModuleStore.cs` |
| 会话与路径 | `Shigure/App/RuntimeSessionCoordinator.cs`、`RandomizedExecutableLauncher.cs`、`Infrastructure/AppPaths.cs` |
...
- 像素门禁：[[docs/40-跨项目/01-Shingen-像素生产消费契约|像素生产消费契约]]
- 配置门禁：[[docs/40-跨项目/02-Shingen-ClassBlocks到config同步契约|ClassBlocks 到 config 同步契约]]
- 热键门禁：[[docs/40-跨项目/03-Shingen-ClassMacros到keymap与按键契约|ClassMacros 到 keymap 与按键契约]]
- 历史审计：[[docs/OPTIMIZATION_zh-CN|Fuyutsui 优化建议]]
